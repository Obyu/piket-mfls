{{--
    Diperbarui: link menu lengkap sebelumnya cuma "Dashboard" generik dan tidak
    menunjuk ke halaman admin/staff yang sebenarnya sudah dibuat (teams, shifts,
    schedules, leads, reports). Sekarang:
    - Desktop (md+): nav ini jadi topbar tipis di sebelah sidebar (menu utama ada di sidebar).
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            <div class="flex items-center md:hidden">
                <a href="{{ request()->routeIs('admin.*') ? route('admin.dashboard') : route('staff.dashboard') }}">
                    <x-application-logo class="block h-9 w-auto fill-current text-sky-500" />
                </a>
            </div>

            <div class="hidden md:flex flex-1 items-center">
                {{-- Ruang topbar: bisa dipakai nanti untuk search bar / breadcrumb --}}
            </div>
            ...
            <!-- Hamburger (mobile only, sidebar menggantikan menu ini di desktop) -->
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden md:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @else
                ...
                <x-responsive-nav-link :href="route('staff.leads.index')" :active="request()->routeIs('staff.leads.*')">
                    {{ __('Leads / Prospek') }}
                </x-responsive-nav-link>
